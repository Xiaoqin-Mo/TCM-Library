...
- [x] PR-A 计划文档 — PR #25 合入 dev
- [x] PR-B 经方条目 — 实际收录 39 首，PR #26 合入 dev；后续补入 4 首（桂枝加葛根汤、大陷胸汤、麻黄附子细辛汤、苓桂术甘汤）于 PR #32，合计 **43 首**，2258 条全绿
- [x] PR-C 上经 — 48 味（去重后），PR #29 合入 dev（2302 条全绿）
- [x] PR-D 中经 — 47 味（去重后），PR #30 合入 dev（2301 条全绿）
- [x] PR-E 下经 — 49 味（去重后），PR #31 合入 dev（2303 条全绿）

**实际收录神农本草经 144 味**（上 48 + 中 47 + 下 49，跨批按品阶去重 6 味），落在 120–180 目标区间。id 统一加 `snb_` 前缀避免与 `library/zhongyao` 药典条目冲突。
