...
- [x] X1 常见病证诊疗指南要点 18 — PR [#48](https://github.com/Xiaoqin-Mo/TCM-Library/pull/48) 已合并，2486→2504
- [x] X2 治未病/体质/适宜技术规范 18 — PR [#49](https://github.com/Xiaoqin-Mo/TCM-Library/pull/49) 已合并，2504→2522（完成）

> 落点修正：计划初稿写 `library/xiandai/zhinan/`，但 `zhinan` 并非 `xiandai` 已注册子类。
> 实际落点：X1 病证指南 → 已注册子类 `zhidao`（临床指南）；X2 治未病/规范 → `jiaocai`（教材与规范）。
> id 编号：X1 病证与库中既有 `<病证>_001` 线条目区隔，统一用 `_002`；X2 主题首现用 `_001`。
> 回归测试：「同字段多值 OR」检查因 ganmao_002 命中「风寒束表」，按「旧基线子集+抽查新增」改为子集断言。
