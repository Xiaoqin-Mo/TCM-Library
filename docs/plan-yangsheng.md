...
## 7. 康复子线扩充（S4，2026-09-24 立项）

- 现状：`yangsheng/kangfu`（康复）子类为 0 条；分类体系已注册（见 `scripts/tcm_lib.py`）。
- 目标：新建**中医康复学要点**约 18 条（`library/yangsheng/kangfu/<条目拼音>/`，type=`zhinan`），
  基线 dev 3101 条 → 落点 ≥3119 条。
- 条目清单（初拟，执行前逐条查重）：中医康复学概述、康复评定（功能/生活能力评定）、
  运动康复（功法结合）、针灸康复、推拿康复、情志康复、食疗康复、中风后康复、骨折术后康复、
  脊髓损伤康复、慢性疼痛康复、老年病康复、心肺疾病康复、肿瘤术后调理康复、儿童脑瘫康复、
  康复护理、气功康复、作业疗法（中医结合）。
- 格式/纪律：同 S1–S3（内容边界同）；涉病康复条目加注「康复方案请在医师/康复师指导下制定，
  本条目为知识参考」。
- 每批收尾：同本计划第 5 节；审核门见 `AGENTS.md`（内容 PR 只含内容、构建产物后置）。
