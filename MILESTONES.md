...
>
> 状态更新于 2026-09-15：全库 **2254 条**（素问 81 + 灵枢 81 + 伤寒论 660（含序 2）+ 金匮要略 25 + 难经 81 + 药典药材 637 + 药典外品种 64 + v0.3 新增 585：经穴 361 + 针灸大成 24 + 诊断学 52 + 内科 47 + 温病 52 + 医案 50 + 经方 39）。
...
**优先级 P1（本草 / 方剂）**
- [x] 《神农本草经》药物条目（`jingdian/bencao/shennong/`）— 完成 144 味（上 48 + 中 47 + 下 49，PR #29/#30/#31，id 加 snb_ 前缀区分药典条目线）
- [x] 《金匮要略》25 篇（`jingdian/jingui/`）
- [x] 《难经》81 难（`jingdian/nanjing/`）
- [x] 经方条目（桂枝汤、麻黄汤、小柴胡汤等 30+，`fangji/jingfang/`）— 完成 43 首（PR #26 收录 39 首，PR #32 补入桂枝加葛根汤、大陷胸汤、麻黄附子细辛汤、苓桂术甘汤 4 首）
...
- [x] 十四经经穴 361 穴（`zhenjiu/shuxue/`）
- [x] 《针灸大成》精选（`zhenjiu/`）
- [x] 《中医诊断学》要点条目（`zhenduan/`）
- [x] 《中医内科学》常见病证 50+（`linchuang/neike/`）
- [x] 温病学：温病条辨、温热论（`jingdian/wenbing/` + `linchuang/wenbing/`）
- [x] 医案 50+（`yishi/yian/`）
...
> 条目总量已达成（2486），其余发布项进行中。

- [x] 现代中医：指南 / 共识 / 教材规范条目（`xiandai/zhidao/` + `xiandai/jiaocai/`，36 条，PR #48–#50）
- [x] 养生康复：导引、食疗、起居条目（`yangsheng/`，42 条，PR #42–#45）
- [x] 医史医家：医家 30+、流派 10+（`yishi/yijia/` + `yishi/xuepai/`，42 条，PR #37–#40）
- [x] 白话提要覆盖全库（全库 2489 个条目均有非空白话提要）
- [ ] CLI 检索器完整（分页 / 多条件 / 排序展示）
- [x] 发布流程：`release/*` 分支 → `main` tag v1.0.0（2026-09-16 发布，2522 条）
